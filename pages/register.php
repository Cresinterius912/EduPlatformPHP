...
$checkEmailStmt = $conn->prepare("SELECT email FROM eduplatformphp WHERE email =?"); //Selects every email from the user data given to check if email given exists
$checkEmailStmt->bind_param("s", $email); 
$checkEmailStmt->execute();
$checkEmailStmt->store_result(); 
//If email is not found in the system it stores the results/email in the system and lets the user continue

if ($checkEmailStmt->num_rows > 0){
    $message = "Email ID Already Exists";
    $toastClass = "#007bff"; //Primary Colour
} else{
    //Prepare and Bind
    $stmt = $conn->prepare("INSERT INTO eduplatform (username, email, password) VALUE (?, ?, ?)");
    $stmt->bind_param("sss", $username, $email, $hashedPassword);

    if ($stmt->execute()){
        $message = "Account Created Successfully";
        $toastClass = "#28a745"; //Positive Colour
    } else{
        $message = "Error: " . $stmt->error;
        $toastClass = "#dc3545"; //Negative Colour
    }
    $stmt->close();
}
